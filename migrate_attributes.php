...
try {
  ...
  if(!checkTable('bv_x_magento_attributes', $mag_dbh)){
    $result = $mag_dbh->query('
      CREATE TABLE `bv_x_magento_attributes` (
...
try {
  ...
  $result = $dbh->query('SELECT * from bvc_ProductProperty');
} catch(PDOException $e) {  
  echo $e->getMessage();
  exit();
}
...
function addAttribute(bvin_id, iteration, max){
  ...
  $.ajax({
    ...
    if(iteration+1 < max){
      addAttribute(bvins[iteration+1], iteration+1, max);
    }
  });
}
</script>
