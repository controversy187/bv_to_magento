...
try {
  ...
  if(!checkTable('bv_x_magento_products_keywords', $mag_dbh)){
...
$(document).ready(function(){
  ...
  addProductKeywords(bvins[0], 0, totalBvins);
});

function addProductKeywords(bvin_id, iteration, max){
  humanNumber = iteration+1;
  $('#responseBlock1').append('<br>' + humanNumber + ' / ' + max + ' : ' + bvin_id + '... ');

  $.ajax({
    url: "update_product_keywords.php",
    type: "POST",
    data: {bvin : bvin_id},
    dataType: "html"
    ...
    if(iteration+1 < max){
      addProductKeywords(bvins[iteration+1], iteration+1, max);
    }
  });
}
</script>
