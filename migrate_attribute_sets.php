...
try {
  ...
  if(!checkTable('bv_x_magento_attribute_sets', $mag_dbh)){
    $result = $mag_dbh->query('
      CREATE TABLE `bv_x_magento_attribute_sets` (
...
try {
  ...
  $result = $dbh->query('SELECT * from bvc_ProductType');
} catch(PDOException $e) {  
  echo $e->getMessage();
  exit();
}
...
$(document).ready(function(){
  ...
  $('#responseBlock1').append('Adding ' + totalBvins + ' Product Attribute Sets<br>');

  addAttributeSet(bvins[0], 0, totalBvins);
});

function addAttributeSet(bvin_id, iteration, max){
  humanNumber = iteration+1;
  $('#responseBlock1').append('<br>' + humanNumber + ' / ' + max + ' : ' + bvin_id + "... ");

  $.ajax({
    url: "add_attribute_set.php",
    type: "POST",
    data: {bvin : bvin_id},
    dataType: "html"
    ...
    if(iteration+1 < max){
      addAttributeSet(bvins[iteration+1], iteration+1, max);
    }
  });
}
</script>
